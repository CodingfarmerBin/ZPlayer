...
import com.zqb.baselibrary.http.cookie.SerializableCookie
...
class SPCookieStore(context: Context) : CookieStore {
    ...
    /** 将url的所有Cookie保存在本地  */
    @Synchronized
    override fun saveCookie(url: HttpUrl, urlCookies: ArrayList<Cookie>) {
    ...
    override fun removeCookie(url: HttpUrl, cookie: Cookie): Boolean {
        ...
        if (!cookies[url.host()]!!.containsKey(cookieToken)) return false
    ...
    companion object {
        ...
        private val COOKIE_PREFS = "rx_http_utils_cookie"           //cookie使用prefs保存
